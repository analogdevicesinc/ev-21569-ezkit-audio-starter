...
static int is25lp_read_uid(const FLASH_INFO *fi, uint32_t addr, uint8_t *buf, int size);
...
static int is25lp_init(const FLASH_INFO *fi)
{
    ...
    /* Read the UID */
    if (result == FLASH_OK) {
        result = is25lp_read_uid(fi, 0x000000, (uint8_t *)fi->UID, sizeof(fi->UID));
    }
...
static int is25lp_read_uid(const FLASH_INFO *fi, uint32_t addr, uint8_t *buf, int size)
{
    ...
    /* Configure the CMD_READ_UID command with 8 dummy cycles */
    cmd[0] = CMD_READ_UID;
    cmd[1] = (addr >> 16) & 0xFF;
    cmd[2] = (addr >> 8) & 0xFF;
    cmd[3] = (addr >> 0) & 0xFF;
    ...
    spiMultiXfer[1].rx = buf;
    spiMultiXfer[1].len = size;
    spiMultiXfer[1].flags = SPI_SIMPLE_XFER_NORMAL_IO;
...
int is25lp_close(const FLASH_INFO *fi)
{
    ...
    /* Put the device back to 3 byte address mode */
    tx[0] = CMD_EXIT_4_BYTE_ADDRESS_MODE;
    spiResult = spi_xfer(fi->flashHandle, 1, NULL, tx);
    if (spiResult != SPI_SIMPLE_SUCCESS) {
        result = FLASH_ERROR;
    }
